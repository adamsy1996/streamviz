...
## Modules
Call `visualize_read_me` again with the type parameter to load detailed guidance:
...
- Box subtitles: ≤5 words. Detail goes in chat follow-up (`sendPrompt`) or the prose below — not the box.
...
### Philosophy
- **Seamless**: Users shouldn't notice where the host app ends and your widget begins.
...
- **Typography**: The default font is the host sans font. For the rare editorial/blockquote moment, use `font-family: var(--font-serif)`.
...
### sendPrompt(text)
A global function that sends a message to the current chat as if the user typed it. Use it when the next step needs the assistant to continue analysis, generate a new view, or drill into an item. Handle filtering, sorting, toggling, and local calculations inside the widget JavaScript instead.

### Links
Use normal `<a href="https://...">` links only when a real external reference is necessary. Do not call `openLink()`; use `sendPrompt(text)` for follow-up actions that should continue the conversation.
...
- Use `sendPrompt()` for any action that should continue the assistant conversation
